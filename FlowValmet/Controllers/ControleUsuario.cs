...
namespace FlowValmet.Controllers
{
    internal class ControleUsuario
    {
        ...
        {
            List<Tuple<int, string, string, string, string>> listaLembretes = new List<Tuple<int, string, string, string, string>>();
            try
            {
                ...
                if (conexao != null)
                {
                    ...
                    while (reader.Read())
                    {
                        listaLembretes.Add(Tuple.Create(Convert.ToInt32(reader["id"]), Convert.ToString(reader["nome"]),
